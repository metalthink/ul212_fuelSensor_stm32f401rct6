...
@ingroup buffer
*/
uint16_t ModbusMaster::getResponseBuffer2(uint8_t u8Index)
{
...
@ingroup register
*/
uint8_t ModbusMaster::readHoldingRegisters2(uint16_t u16ReadAddress,
  uint16_t u16ReadQty)
{
  _u16ReadAddress = u16ReadAddress;
...
uint8_t ModbusMaster::ModbusMasterTransaction(uint8_t u8MBFunction)
{
  ...
  /*
  Serial3.write(u8ModbusADU[0]);
    Serial3.write(u8ModbusADU[1]);
      Serial3.write(u8ModbusADU[2]);
        Serial3.write(u8ModbusADU[3]);
          Serial3.write(u8ModbusADU[4]);
    Serial3.write(u8ModbusADU[5]);
      Serial3.write(u8ModbusADU[6]);
        Serial3.write(u8ModbusADU[7]);
       ...
       for (i = 0; i < 4; i++)
        {
          if (i < ku8MaxBufferSize)
          {
            ...
            _u16ResponseBuffer[i]=u8ModbusADU[i+i+2]<<8 | u8ModbusADU[i+i+3];
             //Serial3.write(_u16ResponseBuffer[0]);          
          }
          _u8ResponseBufferLength = i;
        }
        break; 
